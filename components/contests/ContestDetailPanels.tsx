...
import {
  ScoreBreakdownPanel,
  type BreakdownRow,
} from "@/components/contests/ScoreBreakdownPanel";
import { ContestResultPanel } from "@/components/contests/ContestResultPanel";
import type {
  ContestBonusReward as ContestBonusRewardInput,
  ContestStatus,
  LineupOption,
...
function formatRewardParts(input: {
  pointsAmount: number;
  xpAmount: number;
  packsCount: number;
}) {
  return [
    input.pointsAmount > 0
      ? `${input.pointsAmount.toLocaleString()} pts`
      : null,
    input.xpAmount > 0 ? `${input.xpAmount.toLocaleString()} XP` : null,
    input.packsCount > 0
      ? `${input.packsCount.toLocaleString()} pack${input.packsCount > 1 ? "s" : ""}`
      : null,
  ].filter((value): value is string => Boolean(value));
}

function formatRewardSummary(input: {
  pointsAmount: number;
  xpAmount: number;
  packsCount: number;
}) {
...
function groupRewardTiers(tiers: RewardTier[]): RewardDisplayTier[] {
  ...
  for (const tier of tiers) {
    ...
    if (sameRewardShape) {
      ...
      previous.label =
        previous.rankStart === previous.rankEnd
          ? `Rank ${previous.rankStart}`
          : `Ranks ${previous.rankStart}-${previous.rankEnd}`;
      continue;
    }
...
function buildOpenRewardPool(
  ...
  return {
    pointsPool: tiers.reduce(
      (sum, tier) => sum + Math.max(0, tier.pointsAmount),
      0,
    ),
    packPool: tiers.reduce(
      (sum, tier) => sum + Math.max(0, tier.packsCount),
      0,
    ),
...
export function HeroPanel({
  ...
  return (
    <Surface className="contest-detail-shell-hero" variant="raised">
      <div
        className={`contest-detail-hero-visual${hasCoverImage ? " has-image" : " is-fallback"}`}
        style={heroCoverStyle}
      >
        <div className="contest-detail-hero-visual-badge-row">
          <span className={`mcg-badge ${statusTone}`}>{status}</span>
          <span className="contest-detail-hero-stamp">Arena event</span>
        </div>
        ...
        <div className="contest-detail-hero-copy">
          <div className="contest-detail-hero-copy-panel">
            ...
            {error ? (
              <p className="contest-detail-inline-alert error">{error}</p>
            ) : null}
            {!error && flash ? (
              <p className="contest-detail-inline-alert success">{flash}</p>
            ) : null}
          </div>
        </div>
      </div>
      ...
      <div className="contest-detail-hero-side">
        <div
          className={`contest-detail-timing-panel status-${status.toLowerCase()}`}
        >
        ...
        {primaryAction ? (
          <div className="contest-detail-hero-action-panel contest-detail-hero-action-panel--event">
            <button
              type="button"
              className="mcg-btn primary contest-detail-hero-cta"
            ...
            {helpHref ? (
              <div className="contest-detail-help-panel">
                <p className="contest-detail-help-eyebrow">
...
function InlineLeaderboard({
  ...
  if (rows.length === 0)
    return <p className="contest-detail-empty-note">Ranking pending.</p>;
  ...
  if (rows.length === 0)
    return (
      <p className="contest-detail-empty-note">
        Per-card scoring is not available yet.
      </p>
    );
  ...
  return (
    <div className="contest-breakdown-list">
      {rows.map((row) => (
        <div
          key={row.id}
          className={`contest-breakdown-row${row.dataQuality === "INCOMPLETE" ? " is-incomplete" : ""}`}
        >
          <div className="contest-breakdown-card">
            ...
            {row.cardInstance.cardTemplate.rarity ? (
              <span className="mcg-badge">
                {row.cardInstance.cardTemplate.rarity.code}
              </span>
            ) : null}
            {row.cardInstance.cardTemplate.edition ? (
              <span className="mcg-badge">
                {row.cardInstance.cardTemplate.edition.code}
              </span>
            ) : null}
            {row.dataQuality === "INCOMPLETE" ? (
              <span className="mcg-badge is-warn">Incomplete data</span>
            ) : null}
          </div>
...
export function MainStateBlock({
  ...
  const rewardSummary = myRewards
    ? [
        ...
        myRewards.packsTotal > 0
          ? `${myRewards.packsTotal} pack${myRewards.packsTotal > 1 ? "s" : ""}`
          : null,
      ]
        .filter((value): value is string => Boolean(value))
        .join(" • ")
  ...
  return (
    <Surface
      id="contest-main-experience"
      className="contest-detail-block contest-detail-main-surface"
      variant="raised"
    >
      ...
      {status === "SETTLED" ? (
        <div className="contest-detail-result-lead">
          ...
          <div className="contest-detail-result-rail">
            <span>
              <strong>
                {typeof myScore === "number" ? myScore.toFixed(2) : "—"}
              </strong>
              <small>Score</small>
            </span>
            <span>
      ...
      {status === "LIVE" || status === "SETTLED" ? (
        <section className="contest-detail-main-section">
          <div className="contest-detail-section-head">
            <div>
              <p className="mcg-eyebrow">
                {status === "SETTLED" ? "Leaderboard" : "Standings"}
              </p>
              <h4>
                {status === "SETTLED"
                  ? "Final leaderboard"
      ...
      {(status === "OPEN" || status === "LOCKED") && rankingRows.length > 0 ? (
        <section className="contest-detail-main-section">
          <div className="contest-detail-section-head">
            <div>
              <p className="mcg-eyebrow">
                {status === "OPEN" ? "Field snapshot" : "Early standings"}
              </p>
              <h4>
                {status === "OPEN"
                  ? "Who is already in"
  ...
  return (
    <Surface className="contest-detail-primary-panel" variant="highlight">
      <div className="contest-detail-primary-copy">
        ...
        <p>{body}</p>
        {secondaryText ? (
          <p className="contest-detail-primary-secondary">{secondaryText}</p>
        ) : null}
        {error ? (
          <p className="contest-detail-inline-alert error">{error}</p>
        ) : null}
      ...
      <div className="contest-detail-primary-side">
        {metrics.length > 0 ? (
          <div
            className={`contest-detail-metric-grid metrics-${Math.min(metrics.length, 3)}`}
          >
            {metrics.map((metric) => (
              <article
                key={metric.label}
                className={`contest-detail-metric tone-${metric.tone ?? "neutral"}`}
              >
        ...
        {primaryAction ? (
          <button
            type="button"
            className="mcg-btn primary contest-detail-primary-cta"
            onClick={primaryAction.onClick}
            disabled={primaryAction.disabled}
          >
            {primaryAction.label}
...
export function LineupPanel({
    ...
    <>
      {embedded ? (
        <div className="contest-detail-lineup-meta">
          ...
          <div
            className="contest-detail-lineup-progress-bar"
            aria-hidden="true"
          >
            <span
              style={{
                width: `${Math.min(100, (selectedCount / Math.max(rosterSize, 1)) * 100)}%`,
              }}
            />
          </div>
          ...
          return (
            ...
            >
              ...
              {cardView ? (
                ...
                <span className="contest-command-slot-missing">
              ...
              {isSettled ? (
                <span className="contest-detail-slot-note">
                  {slotCard.finalScore !== null
                    ? `${slotCard.finalScore.toFixed(2)} pts`
                    : "—"}
                </span>
              ) : null}
            </button>
  ...
  if (embedded)
    return <div className="contest-detail-lineup-embed">{content}</div>;
...
export function LeaderboardPanel({
  ...
  const myRow = currentUserId
    ...
    : null;
  const shouldAppendMine =
    compact && myRow && !previewRows.some((row) => row.id === myRow.id);
  ...
  return (
    <Surface
      ...
      className="contest-detail-block leaderboard-panel"
      variant="raised"
    >
      <div className="contest-detail-block-head">
        <div>
...
export function RewardsPanel({
  ...
  const hasEarnedRewards = Boolean(
    myRewards &&
    (myRewards.pointsTotal > 0 ||
      myRewards.xpTotal > 0 ||
      myRewards.packsTotal > 0),
  );
  ...
  const openPoolItems = [
    openPool.pointsPool > 0
      ? {
          label: "Points pool",
          value: `${openPool.pointsPool.toLocaleString()} pts`,
        }
      : null,
    openPool.packPool > 0
      ? {
          label: "Pack pool",
          value: `${openPool.packPool.toLocaleString()} pack${openPool.packPool > 1 ? "s" : ""}`,
        }
      : null,
    openPool.rewardedTopPercent
      ? {
          label: "Paid range",
          value: `Top ${openPool.rewardedTopPercent}% paid`,
        }
      : null,
  ...
  return (
    <Surface className="contest-detail-block rewards-panel" variant="raised">
        ...
        <div className="contest-detail-earned-rewards">
          ...
          <div className="contest-detail-reward-tier-list">
            {parsedBonusRewards.map((reward) => (
              <article
                key={`${reward.targetRank}-${reward.rewardType}-${reward.id ?? reward.note ?? reward.amount}`}
              >
      ...
      {isOpen ? (
          ...
          <>
            ...
            <div className="contest-detail-reward-pool-grid">
              {openPoolItems.map((item) => (
                <article
                  key={item.label}
                  className="contest-detail-reward-pool-card"
                >
                  <span>{item.label}</span>
                  <strong>{item.value}</strong>
                </article>
        ...
        <>
          ...
          {isSettled && myRewards ? (
            <div className="contest-detail-earned-rewards">
              <div className="contest-detail-earned-rewards-head">
                <strong>
                  {hasEarnedRewards ? "Your rewards" : "Your result"}
                </strong>
                <span>
                  {hasEarnedRewards
                    ? formatRewardSummary({
                        pointsAmount: myRewards.pointsTotal,
                        xpAmount: myRewards.xpTotal,
                        packsCount: myRewards.packsTotal,
                      })
                    : "No rewards earned"}
                </span>
              </div>
          ...
          <div
            className="contest-detail-reward-tier-list contest-detail-reward-tier-table"
            role="table"
            aria-label={
              isSettled
                ? "Final reward payout table"
                : "Live reward payout table"
            }
          >
            <article
              className="contest-detail-reward-tier-table-head"
              role="row"
            >
              <div
                className="contest-detail-reward-tier-rank"
                role="columnheader"
              >
                <strong>Placement</strong>
                <span>
                  {isSettled ? "Locked finish" : "Current payout band"}
                </span>
              </div>
              <div
                className="contest-detail-reward-tier-value"
                role="columnheader"
              >
                <strong>Payout</strong>
                <span>
                  {isSettled ? "Rewards earned" : "Exact reward bundle"}
                </span>
              </div>
            </article>
              ...
              return (
                <article key={`${tier.label}-${index}`} role="row">
                  ...
                  <div className="contest-detail-reward-tier-value" role="cell">
                    ...
                    {isSettled && tier.winnerLabel ? (
                      <span>{tier.winnerLabel}</span>
                    ) : null}
                  </div>
                </article>
              );
        ...
        <>
          <p className="contest-detail-panel-copy">
            Reward pool configured. Exact payout rows will appear here once
            rank-based tiers are published.
          </p>
          <div className="contest-detail-reward-pool-grid">
            {openPoolItems.map((item) => (
              <article
...
export function FactsLifecyclePanel_UNUSED({
  ...
  const facts = [
    ...
    ...(leagueTierRequired
      ? [{ label: "League", value: leagueTierRequired }]
      : []),
  ].slice(0, 4);
