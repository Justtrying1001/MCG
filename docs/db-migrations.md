...
6. Si votre base est déjà provisionnée (objets existants) et que la migration unique `init` échoue avec `P3018`/`already exists`, utiliser une baseline contrôlée (`prisma migrate resolve --applied <migration_init>`) avant de relancer le deploy.
7. Vérifier l'intégrité applicative après déploiement.
