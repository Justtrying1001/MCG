...
1. Find an open contest from the contest lobby
2. Select cards from your collection to build your roster
...
Once a contest moves to LOCKED, **your roster is final**. You cannot swap cards, add cards, or withdraw.
...
Each contest has a defined **market window** — a start time and end time. Token performance is measured from the snapshot at lock to the snapshot at settlement.
...
After settlement, rewards are distributed based on final rankings. See [Rewards](rewards.md) for details.
