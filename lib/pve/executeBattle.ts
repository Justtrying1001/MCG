...
export async function executePveBattle(userId: string, selectedCardIds: string[], difficulty: PveDifficulty) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const exhaustCards = await tx.userCard.updateMany({
        where: {
          userId,
          baseCardId: { in: selectedCardIds },
          quantity: { gt: 0 },
          pveExhausted: false,
        },
      ...
      if (exhaustCards.count !== selectedCardIds.length) {
        throw new Error("MCG_PVE_CONCURRENT_CARD_EXHAUST");
      }

      const spendTicket = await tx.user.updateMany({
        where: { id: userId, pveBattleTickets: { gt: 0 } },
        data: {
      ...
      if (spendTicket.count !== 1) {
        throw new Error("MCG_PVE_CONCURRENT_TICKET_SPEND");
      }
    ...
    if (message === "MCG_PVE_CONCURRENT_CARD_EXHAUST") {
      return { error: "Some selected cards are exhausted for PvE today", status: 400 as const };
    }
    if (message === "MCG_PVE_CONCURRENT_TICKET_SPEND") {
      return { error: "No PvE battle tickets left", status: 400 as const };
    }
    throw error;
  }
}
