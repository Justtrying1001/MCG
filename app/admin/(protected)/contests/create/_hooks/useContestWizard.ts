...
export function useContestWizard(initialContestId: string) {
  ...
  const computedDurationHours = useMemo(() => {
    ...
    const delta = new Date(form.endsAt).getTime() - new Date(form.startsAt).getTime();
    if (!Number.isFinite(delta) || delta <= 0) return null;
    return Number((delta / 3600000).toFixed(2));
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setForm((prev) => ({
        ...
        startsAt: toInputDate(contest.liveAt),
  ...
  const allIssues = useMemo(() => {
    ...
    return [...new Set(arr)];
  }, [form.eligibilityMode, form.entryFeeEnabled, payload]);
