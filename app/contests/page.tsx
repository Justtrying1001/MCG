...
function getFeaturedContest(contests: ContestListItem[]) {
  return contests.find((contest) => contest.status === "LIVE")
...
function getSectionEmptyState(group: LobbyGroupKey) {
  if (group === "active") {
    return {
      ...
      description: "The live arena is between rounds. Check upcoming lobbies below.",
    };
  }
  ...
  if (group === "upcoming") {
    return {
      ...
      description: "Fresh lobbies will appear here when the next contest wave opens.",
    };
  }
...
export default function ContestsPage() {
  ...
  const computed = useMemo(() => {
    const upcoming = contests.filter((contest) => contest.status === "OPEN").length;
    const active = contests.filter((contest) => contest.status === "LOCKED" || contest.status === "LIVE").length;
    const completed = contests.filter((contest) => contest.status === "SETTLED").length;
  ...
  const groupedContests = useMemo(() => {
    const active = sortContests(contests.filter((contest) => contest.status === "LOCKED" || contest.status === "LIVE"), "active");
    const upcoming = sortContests(contests.filter((contest) => contest.status === "OPEN"), "upcoming");
    const completed = sortContests(contests.filter((contest) => contest.status === "SETTLED"), "completed");

    return { active, upcoming, completed };
  }, [contests]);

  const featuredContest = useMemo(() => getFeaturedContest(contests), [contests]);
  ...
  return (
    <SiteShell>
      <div className="contest-lobby-layout">
        <Surface className="contest-lobby-hero" variant="raised">
          <div className="contest-lobby-hero-copy">
            ...
            <h1>Contests</h1>
            <p>
              Jump into active clashes, scout the next wave of lobbies, and review the last results board without leaving the arena.
            </p>
            <div className="contest-lobby-hero-chips" aria-label="Contest lobby summary">
          ...
          <div className="contest-lobby-featured">
            <span className="contest-lobby-featured-label">Featured contest</span>
            {featuredContest ? (
              <>
                <strong>{featuredContest.title}</strong>
                <p>
                  {featuredContest.seasonName ?? featuredContest.code} · {featuredContest._count.entries} players · {featuredContest.leagueTierRequired ?? "OPEN"} tier
                </p>
              </>
            ) : (
              <>
                <strong>No featured contest yet</strong>
                <p>The lobby will spotlight the next headline contest as soon as it opens.</p>
              </>
            )}
            <span className="contest-lobby-featured-total">{computed.total} contests loaded</span>
          </div>
        </Surface>
        ...
        {showInitialSkeleton ? (
          ...
          <div className="contest-arena-empty-wrap">
            <EmptyState title="No contests in the lobby" description="New contests will appear here once the next schedule is published." />
          </div>
        ) : (
            ...
            {([
              {
                ...
                subtitle: "The loudest rooms in the arena. Track live battles and locked entries already in motion.",
                contests: groupedContests.active,
              },
              {
                key: "upcoming",
                title: "Upcoming contests",
                subtitle: "Plan ahead, tune your lineup, and grab your seat before team lock.",
                contests: groupedContests.upcoming,
              },
              {
                key: "completed",
                title: "Completed contests",
                subtitle: "Review recent finishes, final placements, and the latest result boards.",
                contests: groupedContests.completed,
              },
            ] satisfies Array<{ key: LobbyGroupKey; title: string; subtitle: string; contests: ContestListItem[] }>).map((section) => {
              const emptyState = getSectionEmptyState(section.key);

              return (
                <section key={section.key} className={`contest-lobby-section contest-lobby-section-${section.key}`}>
                  <SectionHeader
                    eyebrow="Contest state"
                    title={section.title}
                    subtitle={section.subtitle}
                    actions={<Chip label={`${section.contests.length} listed`} />}
                  />
                  ...
                  {section.contests.length === 0 ? (
                    ...
                    <div className="contest-lobby-grid">
                      {section.contests.map((contest) => (
                        <ContestCard key={contest.id} contest={contest} group={section.key} nowTs={nowTs} />
                      ))}
                    </div>
                  )}
                </section>
