...
import { getSettlementPlan } from "@/lib/domain/contests/settlement-plan-runtime";
...
export async function GET(request: NextRequest, { params }: { params: { contestId: string; planId: string } }) {
  ...
  try {
    const result = await getSettlementPlan(params.planId, params.contestId);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    if (error instanceof ContestRuntimeError) {
      return NextResponse.json({ ok: false, error: error.message }, { status: error.status });
    }
    return handleApiError(error, "Cannot load settlement plan");
  }
}
