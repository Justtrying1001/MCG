...
export async function POST(request: NextRequest, { params }: { params: { contestId: string } }) {
  ...
  try {
    const result = await captureStartSnapshot(params.contestId);
    await safeLogAdminAction({
      actionType: "CONTEST_SNAPSHOT_START_CAPTURE",
      module: "CONTESTS",
      status: "EXECUTED",
      actor,
      targetType: "CONTEST",
    ...
    return handleApiError(error, "Cannot capture START snapshot");
  }
}
