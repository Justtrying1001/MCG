...
export async function POST(request: NextRequest) {
  ...
  console.info(`[jobs] contest-live received validSignature=${isValid}`);
  if (!isValid) {
  ...
  try {
    ...
    console.info(`[jobs] contest-live executing contestId=${contestId}`);
    const result = await reconcileContestLifecycleByTime(contestId);
    console.info(`[jobs] contest-live finished contestId=${contestId} finalStatus=${result?.finalStatus ?? "unknown"}`);
