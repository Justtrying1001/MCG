...
export async function POST(request: NextRequest) {
  ...
  console.info(`[jobs] contest-settle received validSignature=${isValid}`);
  if (!isValid) {
  ...
  try {
    ...
    console.info(`[jobs] contest-settle executing contestId=${contestId}`);
    const result = await reconcileContestLifecycleByTime(contestId);
    console.info(`[jobs] contest-settle finished contestId=${contestId} finalStatus=${result?.finalStatus ?? "unknown"}`);
